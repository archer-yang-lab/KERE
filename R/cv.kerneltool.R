cv.kerneltool <- function(x, y, lambda = NULL, pred.loss = c("misclass", 
    ...
    y <- drop(y)
    kerneltool.object <- kerneltool(x, y, lambda = lambda, delta = delta, omega = omega,
        ...)
    lambda <- kerneltool.object$lambda
    # predict -> coef
    nz <- sapply(coef(kerneltool.object, type = "nonzero"), length)
    if (missing(foldid)) 
    ...
    fun <- paste("cv", class(kerneltool.object)[[2]], sep = ".")
