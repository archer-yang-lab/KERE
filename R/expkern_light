! --------------------------------------------------
SUBROUTINE expkern_light (omega, Kmat, Umat, Dvec, &
...
! --------------------------------------------------
    ...
    ! - - - begin - - -
   ONEvec = 1.0D0
   comega = 1.0D0 / MAX(1.0D0-omega, omega)
   npass = 0
   r = y
   alpmat = 0.0D0
   alpvec = 0.0D0
   ...
   ! - - - computing Ku inverse - - - 
      Omega_lam_inv = 1.0D0/(Dvec*Dvec + comega*ulam(l)*Dvec)
      ...
      alpha_loop: DO
         ...
         oalpvec = alpvec
         alpvec =  oalpvec + 0.5D0*comega*(tmp_scalar2*Gscalar*comb_tmp_vec1+comb_tmp_vec2)
         dif = alpvec - oalpvec
         r = r - MATMUL(dif, Ki)
