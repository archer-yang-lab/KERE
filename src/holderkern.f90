! --------------------------------------------------
SUBROUTINE holderkern (delta, Kmat, Umat, Dvec, Ksum, Bmat, &
...
! --------------------------------------------------
    ...
    yi = 1.0D0
    yi(2:(nobs+1)) = y
	mbd = (delta+1.0D0)*(delta+1.0D0)/delta
	decib = delta / (delta + 1.0D0)
	fdr = - decib ** (delta + 1.0D0)
	npass = 0
	r = 0.0D0
	alpmat = 0.0D0
	alpvec = 0.0D0
	...
	! - - - computing Ku inverse - - - 
		...
		DO
		    ...
		    alpvec =  oalpvec + (2*nobs/mbd) * MATMUL(KUinv, & 
	     	          & (-ulam(l)*MATMUL(K0, oalpvec) - 0.5*yi*MATMUL(Ki, phi)/nobs))
		    dif = alpvec - oalpvec
		    r = r + y * MATMUL(dif, Ki)
