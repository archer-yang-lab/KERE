! --------------------------------------------------
SUBROUTINE expkern_precision (omega, Kmat, Umat, Dvec, Ksum, &
...
! --------------------------------------------------
   ...
   alpmat = 0.0D0
   alpvec = 0.0D0
   Bmat = - spread(Ksum,dim=2,ncopies=nobs) &
    & * spread(Ksum,dim=1,ncopies=nobs)/ REAL(nobs)
...
END SUBROUTINE expkern_precision